...
router.route('/process/adduser').post(function(req, res){
    ...
    var age = Number(age);
    console.log('요청 파라미터 : '+ id+', '+password+', '+ name +', '+ age);
    
    addUser(id, name, age, password, function(err, addedUser){
    ...
    console.log('요청 파라미터 : ' + id + ', ' + password);
     ...
     authUser(id, password, function(err, rows){
        ...
        if(rows){
            ...
            req.app.render('form', context, function(err, html){
                if(err){
                    res.writeHead(200,{"Content-Type":"text/html;charset=utf8"});
...
app.get('/process/board', (req,res) =>{
    ...
    fs.readFile('board.ejs','utf-8',(error, data) =>{
        ...
        conn.query('select no, title, writer, hit, DATE_FORMAT(moddate, "%Y/%m/%d %T") as moddate from board1', function(err, rows){
            conn.release();
            if (err){
